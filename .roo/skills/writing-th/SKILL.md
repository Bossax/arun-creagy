...
- **Harness Constraints**:
  ...
  - **ห้าม** บีบอัด ย่อความ หรือตัดเนื้อหารายละเอียด/ตัวอย่างในร่างเดิมออกเป็นอันขาด (Preservation-First Rule)
  - **ห้าม** ใช้ภาษาอังกฤษในวงเล็บสำหรับคำแนวคิด (เช่น `(Data Availability)`) อนุญาตเฉพาะชื่อฟิลด์ข้อมูลจริงในระบบงาน เช่น `DISASTER_EVENT`
  - **ห้าม** ใช้โครงสร้างแปลภาษาอังกฤษ (เช่น `ไม่ได้...แต่...` หรือประโยคปฏิเสธนำหน้าข้อเท็จจริง) และห้ามใช้โครงสร้างกรรมวาจา (Passive Voice เช่น "ถูก...") เมื่อทราบผู้ปฏิบัติงานจริง
  - **ห้าม** ใช้เครื่องมือภายนอกหรือ Script ในการจัดการรุ่นหรือแก้ไขไฟล์ ให้ใช้ native tools (`multi_replace_file_content` หรือ `replace_file_content`) เท่านั้น
...
- **Action**: Main Agent ทำหน้าที่เป็น Auditor ตรวจสอบผลลัพธ์ของ Subagent หรือร่างข้อความของตนเองด้วยเช็คลิสต์:
  - [ ] โครงสร้างหัวข้อและน้ำหนักเนื้อหาตรงตามต้นฉบับ 100% หรือไม่?
  - [ ] ไม่มีส่วนใดถูกย่อ บีบอัด หรือตัดรายละเอียดความหมายและตัวอย่างเชิงปฏิบัติการออก (Preservation-First Check)?
  - [ ] มีการอุปโลกน์ระเบียบวิธีหรือทฤษฎีใหม่ที่ไม่มีในต้นฉบับหรือไม่? (Hallucination Check)
  - [ ] ปราศจากกรรมวาจา (Passive Voice: "ถูก...") โดยเปลี่ยนเป็นประธาน/หน่วยงานทำหน้าที่โดยตรง?
  - [ ] ไม่มีคำภาษาอังกฤษในวงเล็บที่เป็นคำเชิงแนวคิด (เช่น `(Data Availability)`) และล้างคำว่า "เชิง...", "มุ่งเน้น", "อย่างไรก็ตาม" หรือ AI Jargon ออกครบถ้วน?
  - [ ] คำศัพท์ภาษาอังกฤษเชิงการออกแบบได้รับการทดแทนด้วยคำศัพท์สถาบัน (เช่น `use case` -> `กรณีการใช้งาน`, `workflow` -> `ขั้นตอนการทำงาน`, `raster` -> `ขนาดกริด`, `usability` -> `ความพร้อมใช้งาน`)?
  - [ ] คำว่า `DCCE` ถูกแทนด้วยคำว่า `กรมฯ` ในข้อความภาษาไทยหลัก?
