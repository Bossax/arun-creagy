# Handoff: CRDB workshop v2 — invite package + post team-meeting next session

**Date**: 2026-03-30 22:51 (GMT+7)
**Context**: We have a complete “CRDB workshop v2” concept + plan + templates. Next session will start after a team meeting; goal is to finalize the **meeting agenda** and the **stakeholder invite list** (TOR ≥15 orgs) and prepare invitation attachments (concept, expected outcomes, what participants gain, agenda, preparation guide) for sending by Thursday.
...
## Pending
- [ ] Capture team meeting outcomes (decisions, objections, edits requested) and reconcile against:
  - the approved narrative anchor
  - stakeholder-fit constraints and TOR ≥15-org coverage
  - the v2 workshop plan session mechanics
- [ ] Finalize the stakeholder invite list (named organizations + roles), ensuring **15–20 orgs** across required functional clusters.
- [ ] Translate stakeholder-fit requirements into a **per-session attendance matrix** (who must be in which session/breakout).
- [ ] Prepare invitation-ready package for Thursday:
  - concept note (why this workshop, why now)
  - expected outcomes (deliverables: 2-page steering package + website blueprint annex)
  - what participants gain (clear incentives by stakeholder type)
  - agenda (sessions S1–S5)
  - preparation guide (what to read/bring; prompts; pre-work)
- [ ] Create a short slide deck that explains the workshop concept and logic development order (NFCS→CRDB bridge → approved narrative → evidence → design → outputs).

## Next Session
- [ ] Ingest team meeting notes and update the invite list and agenda accordingly.
- [ ] Draft the invitation attachments (concept/outcomes/gains/agenda/prep) as a single package (or separate docs if required by DCCE template).
- [ ] Confirm the “15 orgs” constraint is met as coverage (15–20 target) and record any missing cluster as a risk with mitigation.
- [ ] Produce the slide deck outline (then prepare a PPT version if needed) using the current artifacts as sources.
