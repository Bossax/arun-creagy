...
This document is that narrative, written the same way as the WP6 service narratives: **problem & evidence**, **value if solved**, **blocker today**, followed by **Side Notes** for anything that doesn't directly serve the case. Every claim either cites an in-repo source or is marked as a judgment call.
...
**Blocker today.** Not data access — DCCE data is accessible without difficulty. The real open question is what analytical expertise this product's analytical layer actually needs: deriving defensible trend statistics and a consistent baseline period from raw grid data is its own skill, not something that falls out of the data being available. Separately, the provenance gap — station-interpolated versus reanalysis — still needs confirming, because that choice changes what the derived trend is actually measuring. Neither is a design decision; both are prerequisites the DRD names but doesn't resolve (DRD, REQ-033 "Done when").
