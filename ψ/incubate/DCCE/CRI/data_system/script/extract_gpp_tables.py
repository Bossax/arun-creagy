#!/usr/bin/env python3
r"""
Extract GPP data tables from the CRI bronze intake workbook.

Run from project root with the CRI data_system venv Python, for example:
    .\ψ\incubate\DCCE\CRI\data_system\.venv\Scripts\python.exe .\ψ\incubate\DCCE\CRI\data_system\script\extract_gpp_tables.py

Outputs:
- `gpp_extracts/gpp-67.raw.csv`
- `gpp_extracts/gpp-60-67.raw.csv`
- `gpp_extracts/gpp.manifest.json`
"""

from __future__ import annotations

import csv
import json
import re
import sys
from pathlib import Path

from openpyxl import load_workbook


SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parents[6]
BRONZE_DIR = PROJECT_ROOT / "ψ" / "incubate" / "DCCE" / "CRI" / "data_system" / "data" / "0_bronze" / "2026-06-12_cri_proj_data"
WORKBOOK_PATH = BRONZE_DIR / "CRI Data - GPP.xlsx"
OUTPUT_DIR = BRONZE_DIR / "gpp_extracts"
MANIFEST_PATH = OUTPUT_DIR / "gpp.manifest.json"

SHEET_OUTPUTS = {
    "GPP 67": "gpp-67.raw.csv",
    "GPP 60-67": "gpp-60-67.raw.csv",
}


def normalize_text(value: object) -> str:
    if value is None:
        return ""
    text = str(value).strip()
    text = re.sub(r"\s+", " ", text)
    return text


def row_values(ws, row_number: int, max_cols: int | None = None) -> list[str]:
    upper = max_cols or ws.max_column
    for row in ws.iter_rows(min_row=row_number, max_row=row_number, min_col=1, max_col=upper, values_only=True):
        return [normalize_text(cell) for cell in row]
    return []


def count_non_empty(values: list[str]) -> int:
    return sum(1 for value in values if value)


def is_numeric_like(value: str) -> bool:
    return bool(re.fullmatch(r"[-+]?\d+(?:\.\d+)?(?:\s*-\s*\d+(?:\.\d+)?)?", value))


def is_gpp_code_label(value: str) -> bool:
    return bool(re.fullmatch(r"\d{4}\s*-\s*.+", value))


def is_gpp67_metric_label(value: str) -> bool:
    normalized = normalize_text(value).casefold()
    return normalized in {
        "gross provincial product (gpp)",
        "gpp per capita (baht)",
        "population (1,000 persons)",
    }


def is_gpp60_67_metric_label(value: str) -> bool:
    return is_gpp67_metric_label(value)


def is_year_header_label(value: str) -> bool:
    return bool(re.fullmatch(r"(?:19|20|25)\d{2}(?:[rp])?", normalize_text(value), flags=re.IGNORECASE))


def find_header_row(ws) -> int:
    best_row = 1
    best_score = -1

    for row_number in range(1, min(ws.max_row, 50) + 1):
        values = row_values(ws, row_number)
        non_empty = [value for value in values if value]
        if not non_empty:
            continue

        score = 0
        score += count_non_empty(values) * 5

        unique_ratio = len(set(non_empty)) / len(non_empty)
        if unique_ratio > 0.8:
            score += 10

        alpha_like = sum(1 for value in non_empty if re.search(r"[A-Za-z]", value))
        numeric_like = sum(1 for value in non_empty if is_numeric_like(value))
        score += alpha_like * 2
        score -= numeric_like * 2

        joined = " ".join(non_empty).casefold()
        for keyword in (
            "province",
            "year",
            "gpp",
            "gross provincial product",
            "value",
            "baht",
            "index",
            "name",
            "code",
            "region",
            "ns",
            "northeast",
        ):
            if keyword in joined:
                score += 4

        if row_number <= 10:
            score += 10 - row_number

        if score > best_score:
            best_score = score
            best_row = row_number

    return best_row


def clean_headers(values: list[str]) -> list[str]:
    headers: list[str] = []
    used: dict[str, int] = {}

    for index, value in enumerate(values, start=1):
        header = normalize_text(value) or f"column_{index}"
        count = used.get(header, 0)
        used[header] = count + 1
        if count:
            header = f"{header}_{count + 1}"
        headers.append(header)

    return headers


def trim_trailing_empty_columns(rows: list[list[str]]) -> list[list[str]]:
    if not rows:
        return rows

    last_non_empty_index = -1
    for row in rows:
        for index, value in enumerate(row):
            if value:
                last_non_empty_index = max(last_non_empty_index, index)

    if last_non_empty_index < 0:
        return [[] for _ in rows]

    width = last_non_empty_index + 1
    return [row[:width] for row in rows]


def detect_header_depth(rows: list[list[str]]) -> int:
    if len(rows) < 2:
        return 1

    first_row = rows[0]
    second_row = rows[1]
    third_row = rows[2] if len(rows) >= 3 else []

    second_non_empty = count_non_empty(second_row)
    second_numeric = sum(1 for value in second_row if value and is_numeric_like(value))
    second_label_like = sum(1 for value in second_row if value and re.search(r"[A-Za-zก-๙]", value))
    third_non_empty = count_non_empty(third_row)
    third_numeric = sum(1 for value in third_row if value and is_numeric_like(value))

    if third_non_empty >= 2 and third_numeric < third_non_empty and second_non_empty >= 2:
        return 3

    if second_non_empty >= 2 and second_label_like >= 2 and second_numeric < second_non_empty:
        return 2

    if any(value for value in first_row[1:]) and any(value for value in second_row[1:]):
        return 2

    return 1


def merge_header_rows(header_rows: list[list[str]]) -> list[str]:
    width = max((len(row) for row in header_rows), default=0)
    merged: list[str] = []

    for col_index in range(width):
        parts: list[str] = []
        seen: set[str] = set()
        for row in header_rows:
            value = normalize_text(row[col_index] if col_index < len(row) else "")
            if not value or value in seen:
                continue
            seen.add(value)
            parts.append(value)
        merged.append(" | ".join(parts))

    return merged


def is_footer_or_note_row(row: list[str]) -> bool:
    non_empty = [value for value in row if value]
    if not non_empty:
        return False

    joined = " ".join(non_empty).casefold()
    footer_markers = (
        "source",
        "note",
        "หมายเหตุ",
        "unit",
        "ที่มา",
        "ล้านบาท",
        "สำนักงาน",
    )

    if len(non_empty) <= 2 and any(marker in joined for marker in footer_markers):
        return True

    return False


def extract_gpp60_67_sheet(ws, csv_name: str) -> dict:
    indexed_rows = [(row_number, row_values(ws, row_number)) for row_number in range(1, ws.max_row + 1)]
    indexed_rows = [(row_number, row) for row_number, row in indexed_rows if any(row)]
    indexed_rows = [
        (row_number, row)
        for row_number, row in zip(
            [r for r, _ in indexed_rows],
            trim_trailing_empty_columns([row for _, row in indexed_rows]),
        )
    ]

    if not indexed_rows:
        return {
            "sheet_name": ws.title,
            "error": "No non-empty rows found",
            "header_row_number": 1,
        }

    header_index = next(
        (
            index
            for index, (_, row) in enumerate(indexed_rows)
            if sum(1 for value in row if is_year_header_label(value)) >= 4
        ),
        None,
    )

    first_metric_index = next(
        (
            index
            for index, (_, row) in enumerate(indexed_rows)
            if len(row) >= 2 and is_gpp60_67_metric_label(row[1])
        ),
        None,
    )

    first_code_index = next(
        (
            index
            for index, (_, row) in enumerate(indexed_rows)
            if len(row) >= 2 and row[0] and is_gpp_code_label(row[1])
        ),
        None,
    )

    if header_index is None:
        return extract_sheet(ws, csv_name)

    header_row_number, header_row = indexed_rows[header_index]
    header_rows = [header_row]
    headers = clean_headers(header_row)

    data_start_index = first_code_index if first_code_index is not None else header_index + 1
    data_rows: list[list[str]] = []
    footer_rows: list[list[str]] = []

    for _, row in indexed_rows[data_start_index:]:
        if is_footer_or_note_row(row):
            footer_rows.append(row)
            continue
        data_rows.append(row)

    while data_rows and not any(data_rows[-1]):
        data_rows.pop()

    csv_path = OUTPUT_DIR / csv_name
    with csv_path.open("w", encoding="utf-8-sig", newline="") as fp:
        writer = csv.writer(fp)
        writer.writerow(headers)
        writer.writerows(data_rows)

    return {
        "sheet_name": ws.title,
        "output_csv_path": str(csv_path),
        "worksheet_dimensions": {
            "max_row": ws.max_row,
            "max_column": ws.max_column,
        },
        "header_row_number": header_row_number,
        "header_row_span": 1,
        "header_rows": header_rows,
        "headers": headers,
        "column_count": len(headers),
        "row_count": len(data_rows),
        "preview_first_data_row": data_rows[0] if data_rows else [],
        "footer_rows_skipped": footer_rows,
        "first_code_row_detected": indexed_rows[first_code_index][0] if first_code_index is not None else None,
        "first_metric_row_detected": indexed_rows[first_metric_index][0] if first_metric_index is not None else None,
    }


def extract_gpp67_sheet(ws, csv_name: str) -> dict:
    indexed_rows = [(row_number, row_values(ws, row_number)) for row_number in range(1, ws.max_row + 1)]
    indexed_rows = [(row_number, row) for row_number, row in indexed_rows if any(row)]
    indexed_rows = [(row_number, row) for row_number, row in zip([r for r, _ in indexed_rows], trim_trailing_empty_columns([row for _, row in indexed_rows]))]

    if not indexed_rows:
        return {
            "sheet_name": ws.title,
            "error": "No non-empty rows found",
            "header_row_number": 1,
        }

    first_data_index = next(
        (
            index
            for index, (_, row) in enumerate(indexed_rows)
            if len(row) >= 2 and row[0] and is_gpp_code_label(row[1])
        ),
        None,
    )

    if first_data_index is None:
        return extract_sheet(ws, csv_name)

    header_entries = [
        (row_number, row)
        for row_number, row in indexed_rows[:first_data_index]
        if not is_footer_or_note_row(row)
    ]

    if header_entries:
        header_rows = [row for _, row in header_entries]
        headers = clean_headers(merge_header_rows(header_rows))
        header_row_number = header_entries[0][0]
        header_depth = len(header_rows)
    else:
        header_rows = []
        headers = clean_headers(["จังหวัด", "รายละเอียด", "value"])
        header_row_number = indexed_rows[first_data_index][0]
        header_depth = 0

    data_rows: list[list[str]] = []
    footer_rows: list[list[str]] = []

    for _, row in indexed_rows[first_data_index:]:
        if is_footer_or_note_row(row):
            footer_rows.append(row)
            continue
        data_rows.append(row)

    while data_rows and not any(data_rows[-1]):
        data_rows.pop()

    csv_path = OUTPUT_DIR / csv_name
    with csv_path.open("w", encoding="utf-8-sig", newline="") as fp:
        writer = csv.writer(fp)
        writer.writerow(headers)
        writer.writerows(data_rows)

    return {
        "sheet_name": ws.title,
        "output_csv_path": str(csv_path),
        "worksheet_dimensions": {
            "max_row": ws.max_row,
            "max_column": ws.max_column,
        },
        "header_row_number": header_row_number,
        "header_row_span": header_depth,
        "header_rows": header_rows,
        "headers": headers,
        "column_count": len(headers),
        "row_count": len(data_rows),
        "preview_first_data_row": data_rows[0] if data_rows else [],
        "footer_rows_skipped": footer_rows,
        "first_metric_row_detected": next(
            (
                row_number
                for row_number, row in indexed_rows[first_data_index:]
                if len(row) >= 2 and is_gpp67_metric_label(row[1])
            ),
            None,
        ),
    }


def extract_sheet(ws, csv_name: str) -> dict:
    header_row_number = find_header_row(ws)
    raw_rows = [row_values(ws, row_number) for row_number in range(header_row_number, ws.max_row + 1)]
    raw_rows = trim_trailing_empty_columns(raw_rows)
    non_empty_rows = [row for row in raw_rows if any(cell for cell in row)]

    if not non_empty_rows:
        return {
            "sheet_name": ws.title,
            "error": "No non-empty rows found from detected header row",
            "header_row_number": header_row_number,
        }

    header_depth = detect_header_depth(non_empty_rows)
    header_rows = non_empty_rows[:header_depth]
    headers = clean_headers(merge_header_rows(header_rows))
    data_rows = []
    footer_rows = []

    for row in non_empty_rows[header_depth:]:
        if is_footer_or_note_row(row):
            footer_rows.append(row)
            continue
        data_rows.append(row)

    while data_rows and not any(data_rows[-1]):
        data_rows.pop()

    csv_path = OUTPUT_DIR / csv_name
    with csv_path.open("w", encoding="utf-8-sig", newline="") as fp:
        writer = csv.writer(fp)
        writer.writerow(headers)
        writer.writerows(data_rows)

    return {
        "sheet_name": ws.title,
        "output_csv_path": str(csv_path),
        "worksheet_dimensions": {
            "max_row": ws.max_row,
            "max_column": ws.max_column,
        },
        "header_row_number": header_row_number,
        "header_row_span": header_depth,
        "header_rows": header_rows,
        "headers": headers,
        "column_count": len(headers),
        "row_count": len(data_rows),
        "preview_first_data_row": data_rows[0] if data_rows else [],
        "footer_rows_skipped": footer_rows,
    }


def extract_tables() -> dict:
    manifest: dict = {
        "workbook_path": str(WORKBOOK_PATH),
        "output_manifest_path": str(MANIFEST_PATH),
        "output_dir": str(OUTPUT_DIR),
        "exists": WORKBOOK_PATH.exists(),
        "sheets": [],
    }

    if not WORKBOOK_PATH.exists():
        manifest["error"] = "Workbook not found"
        return manifest

    workbook = load_workbook(WORKBOOK_PATH, read_only=True, data_only=False)
    try:
        manifest["sheet_names"] = list(workbook.sheetnames)

        for sheet_name, csv_name in SHEET_OUTPUTS.items():
            if sheet_name not in workbook.sheetnames:
                manifest["sheets"].append(
                    {
                        "sheet_name": sheet_name,
                        "output_csv_path": str(OUTPUT_DIR / csv_name),
                        "error": "Sheet not found",
                    }
                )
                continue

            ws = workbook[sheet_name]
            if sheet_name == "GPP 67":
                manifest["sheets"].append(extract_gpp67_sheet(ws, csv_name))
            elif sheet_name == "GPP 60-67":
                manifest["sheets"].append(extract_gpp60_67_sheet(ws, csv_name))
            else:
                manifest["sheets"].append(extract_sheet(ws, csv_name))

        return manifest
    finally:
        workbook.close()


def main() -> None:
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    manifest = extract_tables()
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(manifest, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
