...
```mermaid
flowchart TD
  A[Issue analysis and Draft 1 artifacts] --> B[Define subsection units with roles and CLAIM_IDs]
  B --> C[Prepare evidence bundles per subsection ART_PAR_ID and SRC_ID]
  C --> D[Draft isolated subsection v2 text unit]
  D --> E[Local QA check voice, logic, citation readiness]
  E --> F[Integrate subsections back into chapter file]
  F --> G[Run cross-chapter coherence and citation audit]
```
