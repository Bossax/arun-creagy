...
1. **[Lock] Tool-Execution Reflection Lock**: 
   The agent MUST explain its intended action/plan and explicitly ask for the user's confirmation in the current turn. The agent MUST NOT call any state-changing tools (such as write_file, replace_file_content, run_command) until the user responds with approval in a subsequent turn.
