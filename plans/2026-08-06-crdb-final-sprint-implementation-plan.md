...
- **Business NFRs** will NOT be scoped 1:1 with the 1–2 priority use cases that get full Functional Specs. Instead, NFR tables enrich the **9 already-identified high-signal services**: the 8 services in `02_UseCases_FunctionalSpecs/บทสรุปความต้องการใช้งานบริการข้อมูลสารสนเทศด้านภูมิอากาศ_v6.md` (D-043) plus A-BTR (`2026-07-06_btr-me-reporting-pipeline-use-case`). Full Functional Specs (with sign-off, sample data, ECA discipline) stay narrow — only the 1–2 selected as build-next priority. **Superseded 2026-08-15:** the NFR-enrichment sub-track is dropped from WP6 entirely — WP6 is standardized to industry-standard Business Analyst deliverable convention (Service Business Narratives as the business case, Functional Specs as the requirements artifact); NFR-style thresholds don't belong in either. The already-drafted NFR table (`2026-08-14-WP6-Business-NFR-Thresholds-Table.md`) stays on disk as superseded groundwork, not deleted.
...
| WP6  | Use Case & Demand Analysis                        | **Revised 2026-08-07 (Option 1):** (a) NFR thresholds table scoped to the **2 priority services only** (A-BTR, disaster-loss-statistics), not all 9; (b) full Functional Spec + Assumption Log entries + data-specific Acceptance Criteria for **A-BTR + disaster-loss-statistics** (RESOLVED selection — see Open Decision below). **+ layer tagging:** each requirement tagged data-platform vs. web-platform concern. **Superseded 2026-08-15:** (a) dropped entirely — WP6 standardized to industry-standard BA deliverable convention (Service Business Narratives = business case; Functional Specs = requirements artifact); NFR thresholds don't belong in either format. | `06_Use_Case_Demand_Analysis/`                                                    | 2–4            | **In progress — see WP6 Progress Log** |
...
## WP6 Progress Log (2026-08-15)

WP6 is **partially complete** — originally scoped as four pieces. **Superseded 2026-08-15:** the NFR thresholds table is dropped from scope entirely (see Context and WP6 row above — BA-standardization decision), leaving three pieces: Service Business Narratives (done, draft, restructured to a business-case format this session and sealed — see Sealed section below) and the full Functional Spec + layer tagging (not started).

**Dropped from scope (2026-08-15):**

- **Business NFR Thresholds Table** (`2026-08-14-WP6-Business-NFR-Thresholds-Table.md`) — previously covered all 9 high-signal services per WP6's NFR-enrichment sub-track. Dropped entirely per the 2026-08-15 decision to standardize WP6 to industry-standard BA deliverable convention (business case narrative + functional spec, no NFR table). File stays on disk untouched, git history intact; excluded from WP6's active deliverable set going forward.

**Done, draft, not sealed:**

1. **Service Business Narratives** (`2026-08-14-WP6-Service-Business-Narratives.md`) — demand narrative for all 9 services. Revised across three follow-up sessions: (a) 10 factual corrections from Boss's inline review, each grounded against an in-repo source rather than taken at face value; (b) Service 4 reconciled against WP4's sealed DRD (D-068) — names the four blocked requirements (REQ-012/049/050/051) Service 4 unblocks, and flags REQ-051 candidacy (the NESDC/Chulalongkorn methodology as WP4's calculation-manual requirement) as unverified rather than settled; (c) restructured into industry-standard BA business-case format (problem & evidence / value if solved / blocker today / the ask) + a separate Side Notes block per service for design ideas and adjacent concepts.
2. **New, 2026-08-15** — `2026-08-15-WP4-Non-Catalog-Deliverable-Business-Narratives.md`: business narratives for two WP4 DRD deliverables (DEL-1, Thailand Climatology Dashboard; DEL-11, Feedback platform) that sit outside D-043's 8-service catalog entirely, discovered while checking whether the catalog's 8 services actually cover everything WP4 needs justified. Outside WP6's original scope as written, but the same kind of gap this WP exists to close. Same business-case + side-notes format.

**Not started:**

- Full Functional Spec + Assumption Log + data-specific Acceptance Criteria for the 2 priority services (A-BTR, disaster-loss-statistics) — the Service Business Narratives doc explicitly names its own content as "their conceptual foundation, not a substitute for that work."
- Layer tagging (each requirement tagged data-platform vs. web-platform concern) — not yet applied to any WP6 output.

**Also surfaced, not yet actioned:**

- A full service × DRD-requirement traceability pass beyond Service 4 (deferred; Service 2 flagged as the highest-value remaining piece — 5+ requirements citing its resolution ceiling, the entire Appendix B2 investigation, three open decision briefs E-4/E-5/E-6).
- A lighter service → page-section mapping was run across all 8 services (no blocker analysis, just which DRD page-sections each service surfaces on). Found DEL-4 (Slow-Onset Hazards Profile) shares DEL-1's backend and may be a second orphan of the same kind; found roughly half the DRD's content-only deliverables (DEL-3, 6, 7, 8, 9, 10, 14) map to no D-043 service at all. Neither finding is scoped into WP6 or any other WP yet.

**Sealed 2026-08-15:** Service Business Narratives and the WP4 Non-Catalog Deliverable Business Narratives — see Trigger T-048, Change CH-042, Deliverables D-071/D-072.

**Committed:** NFR table (now dropped from scope, see above) and Service Business Narratives (original + Service-4 reconciliation) were committed (`da61a30` and earlier); Service Business Narratives further revised and committed since. The non-catalog deliverable narratives file was committed 2026-08-15.

---

## Open Decision Before WP6 Can Finish — RESOLVED (2026-08-06)

Which **1–2 use cases** get the full Functional Spec (sample data, sign-off, ECA discipline)? Resolved: **A-BTR reporting system + disaster-loss statistics analysis** — the 2 new products from the TOR70 briefing deck's "3+2" grouping (Slide 15, `TOR70_briefing-deck_slide-text.md`). The "3" in that grouping are DCCE's existing tools, which DCCE itself owns the enhancement-requirements for — out of CRDB's Functional Spec scope. CRDB's own scope centers on the 2 new products it proposed, which have no other owner, and the count (2) matches the 1–2 cap exactly. No further narrowing needed. ~~(The NFR-enrichment sub-track still proceeds on all 9 services regardless.)~~ **Superseded 2026-08-15:** NFR-enrichment sub-track dropped from WP6 entirely — see Context and WP6 row above.
