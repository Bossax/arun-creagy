...
WP6 is **partially complete** — two of its four originally scoped pieces exist as drafts; the full Functional Spec and layer tagging haven't started.

**Done, draft, not sealed:**

1. **Business NFR Thresholds Table** (`2026-08-14-WP6-Business-NFR-Thresholds-Table.md`) — covers all 9 high-signal services (D-043's 8 + A-BTR), per WP6's NFR-enrichment sub-track (which runs on all 9 regardless of which 2 get full specs, per the Open Decision note below).
2. **Service Business Narratives** (`2026-08-14-WP6-Service-Business-Narratives.md`) — demand narrative for all 9 services. Revised across two follow-up sessions: (a) 10 factual corrections from Boss's inline review, each grounded against an in-repo source rather than taken at face value; (b) Service 4 reconciled against WP4's sealed DRD (D-068) — names the four blocked requirements (REQ-012/049/050/051) Service 4 unblocks, and flags REQ-051 candidacy (the NESDC/Chulalongkorn methodology as WP4's calculation-manual requirement) as unverified rather than settled.
3. **New, 2026-08-15, not yet committed** — `2026-08-15-WP4-Non-Catalog-Deliverable-Business-Narratives.md`: business narratives for two WP4 DRD deliverables (DEL-1, Thailand Climatology Dashboard; DEL-11, Feedback platform) that sit outside D-043's 8-service catalog entirely, discovered while checking whether the catalog's 8 services actually cover everything WP4 needs justified. Outside WP6's original scope as written, but the same kind of gap this WP exists to close.
...
**Not yet sealed:** none of WP6's outputs are in the 4 ledgers — all exist as plain drafts pending Boss's `/seal` call.

**Committed:** NFR table and Service Business Narratives (original + Service-4 reconciliation) are committed (`da61a30` and earlier). The new non-catalog deliverable narratives file is untracked, not yet committed.
